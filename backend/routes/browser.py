...
# Determine which HTML parser to use
try:
    ...
    HTML_PARSER = "html.parser"  # Built-in parser, always available
...
async def proxy_page(
    ...
    use_proxy: bool = Query(True, description="Whether to use a proxy"),
):
    """
    ...
    try:
            ...
            for proxy in proxies_to_try[:3]:
                try:
                    if proxy.protocol == "http":
                        ...
                        # SOCKS proxy (only if httpx-socks is available)
                        if not SOCKS_AVAILABLE:
                            print(
                                f"Warning: SOCKS proxy requested but httpx-socks not installed. Skipping proxy {proxy.ip}:{proxy.port}"
                            )
                            continue
        ...
        if "text/html" not in content_type:
            raise HTTPException(
                status_code=400, detail="Only HTML content can be proxied"
            )

        html_content = response.text
        base_url = str(response.url)

        # Parse HTML with BeautifulSoup (use lxml if available, otherwise html.parser)
